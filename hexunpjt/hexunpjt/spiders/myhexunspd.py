...
#             }
#         yield Request('http://' + str(self.uid) + '.blog.hexun.com/p1/default.html',headers=header)
#
#     def parse(self, response):
#         item = HexunpjtItem()
#         item['name'] = response.xpath("//span[@class='ArticleTitleText']/a/text()").extract()
#         item['url'] = response.xpath("//spam[@class='ArticleTitleText']/a/@herf").extract()
#         #接下来需要使用urllib和re模块获取博文的评论数和点击数
#         #首先提取存储评论数和点击数网址的正则表达式
#         pat1 = '<script type="text/javascript" src="(http://click.toolhexun.com/.*?)">'
#         #模拟成浏览器
...
#         #通过正则表达式获取到的数据为一个list，倒数第二个元素为总页数
#         data2 = re.compile(pat4).findall(str(response.body))
#         if (len(data2) >= 2) :
#             totalurls = data2[-2]
#         else:
#             totalurls = 1
#         for i in range(2,int(totalurls)+1):
#             #构造下一次要爬取的url，爬取下一页博文列表中的数据
#             nexturl = 'http://' + str(self.uid) + '.blog.hexun.com/p'+str(i) + '/default.html'
#             #进行下一次爬取，下一次爬取仍然模拟浏览器进行
#             yield Request(nexturl,callback=self.parse,headers={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.108 Safari/537.36'})
